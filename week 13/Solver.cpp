...
int main(int argc, char* argv[])
{
    ...
    vector <float> initial_state=js1["initial_state"];
    dt=js1["dt"];
    T=js1["T"];
    f1.close();
    ofstream f2(argv[2]);
    Double_Math_Pendulum dmp (initial_state, params, dt);
    RK4_method <Double_Math_Pendulum> RK4dmp(dmp, T);
    vector <vector<float>> r=RK4dmp.result();
    vector <float> x1;
    for (auto &i: r[1])
    {
        x1.push_back(params[1]*sin(i));
    }
    x1.shrink_to_fit();
    vector <float> y1;
    for (auto &i: r[1])
    {
        y1.push_back(-params[1]*cos(i));
    }
    x1.shrink_to_fit();
    vector <float> x2;
    for (int i=0; i<r[2].size(); i++)
    {
        x2.push_back(params[1]*sin(r[1][i])+params[2]*sin(r[2][i]));
    }
    x2.shrink_to_fit();
    vector <float> y2;
    for (int i=0; i<r[2].size(); i++)
    {
        y2.push_back(-params[1]*cos(r[1][i])-params[2]*cos(r[2][i]));
    }
    y2.shrink_to_fit();
    json js2={};
    js2["x1"]=x1;
